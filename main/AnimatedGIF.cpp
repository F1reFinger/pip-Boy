...
// --- ESP-IDF Compatibility Patch ---
#include "freertos/FreeRTOS.h"
#include "freertos/task.h"
#include "esp_timer.h" // This will work now that we updated CMakeLists.txt

#ifndef millis
#define millis() (long)(esp_timer_get_time() / 1000)
#endif
...
// --- C Bridge Functions for decode_gif.c ---
// --- Fixed C Bridge Functions for ESP-IDF ---
extern "C" {

    void GIF_begin(GIFIMAGE *pGIF, unsigned char ucPaletteType) {
        AnimatedGIF temp;
        temp.begin(ucPaletteType);
        // Use the new getter to copy the initialized struct
        memcpy(pGIF, temp.getInternalGIF(), sizeof(GIFIMAGE));
    }

    int GIF_openFile(GIFIMAGE *pGIF, const char *szFilename, GIF_DRAW_CALLBACK *pfnDraw) {
        // Cast the pGIF back to the Class to use the library's built-in open logic
        AnimatedGIF *gif = (AnimatedGIF *)pGIF; 
        return gif->open(szFilename, GIFOpenFile, GIFCloseFile, GIFReadFile, GIFSeekFile, pfnDraw);
    }

    int GIF_playFrame(GIFIMAGE *pGIF, int *delayMilliseconds, void *pUser) {
        AnimatedGIF *gif = (AnimatedGIF *)pGIF;
        // false = don't use internal delay, we handle timing in decode_gif.c
        return gif->playFrame(false, delayMilliseconds, pUser);
    }
